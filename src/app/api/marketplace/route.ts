 ...
 */
export async function GET(request: NextRequest): Promise<NextResponse<MarketplaceListResponse | ErrorResponse>> {
  try {
    ...
    const mappedWishes: MarketplaceWishResponse[] = result.wishes.map(wish => ({
      id: wish.id,
    ...
    return NextResponse.json(
      {
        ...
        message: 'An unexpected error occurred while retrieving marketplace wishes'
      },
      { status: 500 }
    );
  }
